...
public class damip implements oajava.sql.ip
{
    ...
    private int iFetchSize;  /* records to return in cursor mode */
    ...
    private List<String>     custCondList = new LinkedList<String>();
    private Iterator<String> custCondItr;
    private String           custCondStr;

    /* column handles */
    private long hcolName, hcolIntVal, hcolDoubleVal, hcolTime, hcolCustCond; /* DAM_HCOL */
        ...
        switch (iInfoType)
                {
                    ...
                    case IP_INFO_COND_LIST_NORMALIZATION_LIMIT:
                    	/* condition list normalization limit */
                    	str = "2";
                    	break;
    ...
    public int ipExecute(long dam_hstmt, int iStmtType, long hSearchCol,xo_long piNumResRows)
        {
            ...
            if (iStmtType == DAM_SELECT || iStmtType == DAM_UPDATE || iStmtType == DAM_DELETE || iStmtType == DAM_INSERT)
            {
                ...
                hcolCustCond = jdam.dam_getCol(dam_hstmt,"CUSTOM_CONDITION");
            ...
            {
            	xo_int bPartialLists = new xo_int(0);
                long pMemTree = jdam.dam_getMemTree(dam_hstmt);
                custCondList.clear();
            	
            	long pSetOfCustCondList = jdam.dam_getSetOfCustomConditionLists(dam_hstmt, bPartialLists);

            	if (bPartialLists.getVal() == 1) {
            		jdam.trace(m_tmHandle, UL_TM_ERRORS, "Query has partial Custom Conditions.\n");
            		jdam.dam_addError(0, dam_hstmt, DAM_IP_ERROR, 0, "Query has partial Custom Conditions.");
            		return IP_FAILURE;
            	}
            	
            	if (pSetOfCustCondList != 0) {
            		int		custCondListIndx = 0, custCondIndx = 0, paramIndx = 0;
            		long	hCond;
            		long	hValExp;
            		xo_long	hCol = new xo_long(0);
            		xo_int iOpType = new xo_int(0);
            		StringBuffer scalarName = new StringBuffer();
            		StringBuffer colName = new StringBuffer();
            		xo_long	hValExpList = new xo_long(0);
            		long	valExpList = 0;
            		xo_int	iValStatus = new xo_int(0);
            		Object	pInputStr = null;
            		long	pCustCondList = jdam.dam_getFirstCondList(pSetOfCustCondList);

            		while (pCustCondList != 0) {
            			++custCondListIndx;
        				custCondIndx = 0;
        				hCond = jdam.dam_getFirstCond(dam_hstmt, pCustCondList);
        				while(hCond !=0) {
        					++custCondIndx;
        					iRetCode = jdam.dam_describeCustomCond(dam_hstmt, hCond, hCol, iOpType, scalarName, hValExpList);
        					valExpList = hValExpList.getVal();
        					jdam.dam_describeCol(hCol.getVal(), null, colName, null, null);
        					
        					String sBuffer = String.format("CL#%02d, CN#%02d, COL:%s, OP#%02d, Fun:%s",
        							custCondListIndx, custCondIndx, colName, iOpType.getVal(), scalarName);

        					hValExp = jdam.dam_getFirstValExp(valExpList);
        					paramIndx = 0;
        					while (hValExp != 0) {
        						// For each param to the custom scalar
        						++paramIndx;
        						pInputStr = jdam.dam_getValueOfExp(pMemTree, valExpList, hValExp, XO_TYPE_CHAR, iValStatus);
        						sBuffer = String.format("%s, P%02d:%s", sBuffer, paramIndx, pInputStr.toString());
        						hValExp = jdam.dam_getNextValExp(valExpList);
        					}
        					custCondList.add(sBuffer);

        					hCond = jdam.dam_getNextCond(dam_hstmt, pCustCondList);
        				}
            			jdam.dam_setOption(DAM_CONDLIST_OPTION, pCustCondList, DAM_CONDLIST_OPTION_EVALUATION, DAM_PROCESSING_OFF);
            			pCustCondList = jdam.dam_getNextCondList(pSetOfCustCondList);
            		}
            	}
            	custCondItr = custCondList.iterator();
            	if (custCondItr.hasNext())
            		custCondStr = custCondItr.next();
            	else
            		custCondStr = "";
        ...
        case DAMOBJ_TYPE_COLUMN:
            {
                ...
                if (IsMatchingColumn(pColSearchObj, OA_CATALOG_NAME, OA_USER_NAME, "CURVALUE")) 
                {
                    ...
                    ColumnObj.SetObjInfo(OA_CATALOG_NAME,OA_USER_NAME,"CURVALUE","CUSTOM_CONDITION",(short)12,"VARCHAR",512,256,(short)DAMOBJ_NOTSET,(short)DAMOBJ_NOTSET,
                    		(short)XO_NULLABLE,(short)DAMOBJ_NOTSET,null,null,(short)SQL_PC_NOT_PSEUDO,(short)0,"Custom Condition");
                    jdam.dam_add_schemaobj(pMemTree,iType,pList,pSearchObj,ColumnObj); 
                }
            }
            break;
    ...
    public long java_build_row(long dam_hstmt, String sName, int iIntVal, double dDoubleVal, long lTime)
        {
            ...
            if (iRetCode != DAM_SUCCESS) return 0;
            iRetCode = jdam.dam_addCharValToRow(dam_hstmt, hrow, hcolCustCond, custCondStr, XO_NTS);
            if (iRetCode != DAM_SUCCESS) return 0;
    ...
    {
        scalar_function[] MyFuncs = new scalar_function[6];
	...
	MyFuncs[3] = new scalar_function("IPNAME",1,"ip_func_ipname",XO_TYPE_CHAR,25,25,0,-3);
        MyFuncs[4] = new scalar_function("NEAR");
        MyFuncs[5] = new scalar_function("ANY");
        return MyFuncs;
    }
    ...
    {
    	long hVal;
    	hVal=jdam.dam_createVal(pMemTree, XO_TYPE_CHAR,"Java Module, CustomScalar IP", XO_NTS);
    	return hVal;
    }
