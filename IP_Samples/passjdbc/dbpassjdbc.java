...
public class dbpassjdbc implements oajava.sql.ip
{
    ...
    final static int MAX_CHAR_VARCHAR_LEN = 8000;
	...
	private boolean         m_bConcatNullYieldsNull;
    ...
    public String ipGetInfo(int iInfoType)
    {
        ...
        switch (iInfoType)
		   ...
		   case IP_INFO_CONCAT_NULL_YIELDS_NULL:
					if ( m_bConcatNullYieldsNull )
                            str = "1"; /* true */
                    else
                            str = "0"; /* false */
                    break;
    ...
    public scalar_function[] ipGetScalarFunctions()
    {
    	try
    	{
        scalar_function[] MyFuncs = new scalar_function[7];
        
       	MyFuncs[0] = new scalar_function("CONVERT", 1, "ip_func_convert_3", -900+1,3);
		MyFuncs[1] = new scalar_function("CONVERT", 1, "ip_func_convert_2", -900+1,2);
		MyFuncs[2] = new scalar_function("ADD", 1, "ip_func_add", XO_TYPE_VARCHAR, 2);
		MyFuncs[3] = new scalar_function("Integer","ADD", 1, 0, "ip_func_Integer_add", XO_TYPE_VARCHAR, DAM_NOT_SET, DAM_NOT_SET, DAM_NOT_SET, 2);
		MyFuncs[4] = new scalar_function("Integer","SUBSTRACT", 1, 0, "ip_func_Integer_substract", XO_TYPE_VARCHAR, DAM_NOT_SET, DAM_NOT_SET, DAM_NOT_SET, 2);
		MyFuncs[5] = new scalar_function("String","ADD", 1, 0, "ip_func_string_add", XO_TYPE_VARCHAR,DAM_NOT_SET, DAM_NOT_SET, DAM_NOT_SET, 2);
		MyFuncs[6] = new scalar_function("String","SUBSTRACT", 1, "ip_func_string_substract", XO_TYPE_VARCHAR, 2);
	...
	public long ip_func_add(long hstmt, long pMemTree, long hValExpList) {
        ...
        hVal = jdam.dam_createVal(pMemTree, XO_TYPE_VARCHAR, "In ip_func_add()", XO_NTS);
        
		return hVal;
	}

	public long ip_func_Integer_add(long hstmt, long pMemTree, long hValExpList) {
		long hVal = 0;
		
		/* create the value to return */
        hVal = jdam.dam_createVal(pMemTree, XO_TYPE_VARCHAR, "In ip_func_Integer_add()", XO_NTS);

		return hVal;
	}

	public long ip_func_Integer_substract(long hstmt, long pMemTree, long hValExpList) {
		long hVal = 0;


		/* create the value to return */
        hVal = jdam.dam_createVal(pMemTree, XO_TYPE_VARCHAR, "In ip_func_Integer_substract()", XO_NTS);
	...
	public long ip_func_string_add(long hstmt, long pMemTree, long hValExpList) {
		...
		/* create the value to return */
        hVal = jdam.dam_createVal(pMemTree, XO_TYPE_VARCHAR, "In ip_func_string_add()", XO_NTS);
        
        
		return hVal;
	}

	public long ip_func_string_substract(long hstmt, long pMemTree, long hValExpList) {
		long hVal = 0;

		/* create the value to return */
        hVal = jdam.dam_createVal(pMemTree, XO_TYPE_VARCHAR, "In ip_func_string_substract()", XO_NTS);
        
		return hVal;
	}

	public long ip_func_convert_3(long hstmt, long pMemTree, long hValExpList) {
		long hVal = 0;

		return hVal;
	}
	
	public long ip_func_convert_2(long hstmt, long pMemTree, long hValExpList)
    {
		long hVal = 0;			
...
*********************************************************************************************/       
    ...
    private int ip_read_configuration()
    {
	    ...
	    /* If 1, use bulk fetch if 0, normal mode */
			...
			/* Get ConcatNullYieldsNull flag from ini */
			sTemp = jdam.getProfileString("PASSIP", "ConcatNullYieldsNull", "0", m_sIniFile.toString());
            iTemp = new Integer(sTemp).intValue();
            if (iTemp == 1)
                m_bConcatNullYieldsNull = true;
            else
                m_bConcatNullYieldsNull = false;
...
*********************************************************************************************/
    ...
    /* check if query cannot use orginal select expression */
            if (!m_bPushPostProcessing && jdam.damex_getParentQuery(hquery) == 0) 
            {
    ...
    htable = jdam.damex_getFirstTable(hquery);
            ...
            {
                if ((piSetQuantifier.getVal() == SQL_SELECT_DISTINCT) && (m_bPushPostProcessing || jdam.damex_getParentQuery(hquery) != 0))
            ...
            if ((phGroupValExpList.getVal() != 0) && (m_bPushPostProcessing || jdam.damex_getParentQuery(hquery) != 0)) 
            {
                pSqlBuffer.append(" GROUP BY ");
                ip_format_group_list(hquery, phGroupValExpList.getVal(), pSqlBuffer);
            }
            if ((phHavingExp.getVal() != 0) && (m_bPushPostProcessing || jdam.damex_getParentQuery(hquery) != 0)) 
            {
                pSqlBuffer.append(" HAVING ");
                ip_format_logexp(hquery, phHavingExp.getVal(), pSqlBuffer);
            }

            if ((phOrderValExpList.getVal() != 0) && (m_bPushPostProcessing || jdam.damex_getParentQuery(hquery) != 0)) 
            {
...
*********************************************************************************************/
    ...
    int     ip_format_col(long hquery, long hCol, StringBuffer pSqlBuffer)
    {
        ...
        /* Since iNestedTableNum returning as 0 for NORMAL tables, iTableNum is updating with iNestedTableNum. Resulting this, 
          for correlated column table number setting as 0 instead sequence number of the table in the join */
...
int ip_format_date_funtions_valexp_list(long hquery, long hValExpList, StringBuffer pSqlBuffer)
 {
        long    hValExp;
        int     iFirst = TRUE;
        StringBuffer      wsAsColName = new StringBuffer(DAM_MAX_ID_LEN+1);
	/*Note :DATETRUNC syntax has to be handled according to Data Source .
			For Example : 
			Postgres supports datetrunc with date_trunc(datepart, timestamp)::date_trunc('hour', timestamp '2001-02-16 20:38:40')
			timestamp token has to be added before date*/
        hValExp = jdam.damex_getFirstValExp(hValExpList);
        ip_format_valexp_as_identifier(hquery,hValExp,pSqlBuffer);
        hValExp = jdam.damex_getNextValExp(hValExpList);
...
int ip_format_valexp_as_identifier(long hquery, long hValExp, StringBuffer pSqlBuffer)
	{
        StringBuffer    sName;
        xo_int      piType = new xo_int(0); /* literal value, column, +, -, *, / etc   */
        xo_int      piFuncType = new xo_int(0);
        ...
        piValStatus = new xo_int();
		    ...
		    pData = jdam.damex_describeVal(hVal.getVal(), piType,
											piXoType,
											piValLen,
											hCol,
		...
		switch (iXoType) 
                {
                    ...
                    case XO_TYPE_VARCHAR:
						...
						pSqlBuffer.append(strObject);
                        break;
				}
		return IP_SUCCESS;
	}
int 	ip_format_scalar_convert(long hquery, long hValExpList, StringBuffer pSqlBuffer)
{
	long            hValExp;
    xo_int      piType = new xo_int(0); /* literal value, column, +, -, *, / etc   */
    xo_int      piFuncType = new xo_int(0);
    ...
    {
        StringBuffer    sName, sQualiferName;
        long            hValExp;
        ...
        sName = new StringBuffer(ip.DAM_MAX_ID_LEN + 1);
        sQualiferName = new StringBuffer(ip.DAM_MAX_ID_LEN + 1);
        hValExp = jdam.damex_getFirstValExp(hValExpList);
        ip_format_valexp(hquery, hValExp, pSqlBuffer);

        jdam.dam_describeScalarEx2(hValExpList, sQualiferName, sName, null, iResXoType, iLength, iPrecision, iScale);
    ...
    {
        StringBuffer    sName;
        StringBuffer    sQualifierName;
        xo_long      phValExpList;

        phValExpList = new xo_long();
        sName = new StringBuffer(ip.DAM_MAX_ID_LEN + 1);
        sQualifierName = new StringBuffer(ip.DAM_MAX_ID_LEN + 1);
        
        jdam.damex_describeScalarValExpEx2(hScalarValExp, sQualifierName, sName, phValExpList);
        ...
        if (sName.toString().equalsIgnoreCase("CONVERT")) 
        {
			//If Backend supports 'fn' prefixed scalar funtion syntax ,please add below line and ({})braces accordingly.
	...
	/*Handle DATEADD ,DATEDIFF, DATETRUNC*/ 		
		else if (sName.toString().equals("DATEADD") || 
		         sName.toString().equals("DATEDIFF") ||
				 sName.toString().equals("TIMESTAMPDIFF") ||
				 sName.toString().equals("TIMESTAMPADD"))
		{
			pSqlBuffer.append(sName);
			pSqlBuffer.append("( ");
			if (phValExpList.getVal() != 0)
				ip_format_date_funtions_valexp_list(hquery, phValExpList.getVal(), pSqlBuffer);			
			pSqlBuffer.append(")" );					 
		}	
	/*Note :DATETRUNC syntax has to be handled according to Data Source .
			For Example : 
			Postgres supports datetrunc with date_trunc(datepart, timestamp)::date_trunc('hour', timestamp '2001-02-16 20:38:40')
			sName needs to be date_trunc*/		
		else if(sName.toString().equals("DATETRUNC") || sName.toString().equals("TIMESTAMPTRUNC"))
		{
			pSqlBuffer.append(sName);
			//pSqlBuffer.append("date_trunc");
			pSqlBuffer.append("( ");
			if (phValExpList.getVal() != 0)
				ip_format_date_funtions_valexp_list(hquery, phValExpList.getVal(), pSqlBuffer);			
			pSqlBuffer.append(")" );					 
		}
		else if(sName.toString().equalsIgnoreCase("current_timestamp"))
		{
			/*Some scalar functions are expected to be with out braces*/
            pSqlBuffer.append(sName);
		}		
        else 
        {
        	if (!sQualifierName.toString().equalsIgnoreCase(""))
        	{
        		pSqlBuffer.append(sQualifierName);
        		pSqlBuffer.append(".");
        	}
        ...
        switch (iType) 
        {
            ...
            case SQL_VAL_LITERAL: /* value is a Xo Type literal */
                ...
                switch (iXoType) 
                {
                    ...
                    case XO_TYPE_DATE:
                        xoTime = (xo_tm)pData;
                        pSqlBuffer.append(" '").append(xoTime.getVal(xo_tm.YEAR)).append("-").append(xoTime.getVal(xo_tm.MONTH)+1).append("-").append(xoTime.getVal(xo_tm.DAY_OF_MONTH)).append("'");
                        break;
                    case XO_TYPE_TIME:
                        xoTime = (xo_tm)pData;
                        pSqlBuffer.append(" '").append(" ").append(xoTime.getVal(xo_tm.HOUR)).append(":").append(xoTime.getVal(xo_tm.MINUTE)).append(":").append(xoTime.getVal(xo_tm.SECOND)).append("'");
                        break;
                    case XO_TYPE_TIMESTAMP:
                        ...
                        if (xoTime.getVal(xo_tm.FRACTION) > 0) 
                        {
                            ...
                            pSqlBuffer.append("'").append(xoTime.getVal(xo_tm.YEAR)).append("-").append(xoTime.getVal(xo_tm.MONTH)+1).append("-").append(xoTime.getVal(xo_tm.DAY_OF_MONTH))
...
************************************************************************/
		...
		try {
			while (true) {
				try {
					...
					jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
							"generateCatalogList():Exception in execute(): " + e + "\n");
				}
			}
		} finally {
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generateCatalogList(): " + e + "\n");
			}
		...
		try {
			while (true) {
				try {
					...
					jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
							"generateSchemasList():Exception in execute(): " + e + "\n");
				}
			}
		} finally {
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generateSchemasList(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }
    ...
    public int generateTableList(long pMemTree,int iType, long pList, Object pSearchObj)
    {
        ...
        if(pSearchObj != null) 
        {
			...
			jdam.trace(m_tmHandle, UL_TM_MAJOR_EV, "empty search object.\n"); 
		}
		...
		try {
			while (true) {
				...
				try {
					if (!rs.next()) {
						jdam.trace(m_tmHandle, UL_TM_MAJOR_EV, "No more rows.\n");
						break;
					}

					sTableType = (0 == m_JdbcUtil.getColumnValue(rs, 4).compareToIgnoreCase("VIEW")) ? "TABLE"
							: m_JdbcUtil.getColumnValue(rs, 4);

					TableObj.SetObjInfo(m_JdbcUtil.getColumnValue(rs, 1), m_JdbcUtil.getColumnValue(rs, 2),
							m_JdbcUtil.getColumnValue(rs, 3), sTableType, null, null, null,
							m_JdbcUtil.getColumnValue(rs, 5));
					jdam.dam_add_schemaobj(pMemTree, iType, pList, pSearchObj, TableObj);
				} catch (Exception e) {
			...
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS, "Error while closing resultset in generateTableList(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }

    public int remapJdbcTypeForVarcharMax(ResultSet rs)
    {
      // OASL-590: MS SQLServer returns VARCHAR for VARCHAR(MAX).
      // Mapping it to LONGVARCHAR for correct behavior
      // We do this for CHAR/VARCHAR types with length > MAX_CHAR_VARCHAR_LEN
      int length = m_JdbcUtil.getIntColumnValue(rs,8);
      int jdbcType = m_JdbcUtil.getIntColumnValue(rs,5);
      if ((jdbcType == Types.VARCHAR || jdbcType == Types.CHAR) && length > MAX_CHAR_VARCHAR_LEN)
		...
		try {
			while (true) {
				try {
					if (!rs.next()) 
						break;				
					int jdbcType = remapJdbcTypeForVarcharMax(rs);
					 ...
					 ColumnObj.SetObjInfo(m_JdbcUtil.getColumnValue(rs,1),m_JdbcUtil.getColumnValue(rs,2),
			                    ...
			                    (short)MapJdbcTypeToXoType(jdbcType),
			                    MapJdbcTypeToXoTypeName(jdbcType),m_JdbcUtil.getIntColumnValue(rs,7),
			...
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generateColumnList(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }
		...
		try {
			while (true) {
				try {
					...
					jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
							"generatePrimaryKeys():Exception in execute(): " + e + "\n");
			...
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generatePrimaryKeys(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }
    ...
    public int generateForeignKeys(long pMemTree,int iType, long pList, Object pSearchObj)
    {
		...
		try {
			while (true) {
				try {
					...
					FkeyObj.SetObjInfo(m_JdbcUtil.getColumnValue(rs, 1), m_JdbcUtil.getColumnValue(rs, 2),
							...
							(short) m_JdbcUtil.getShortColumnValue(rs, 9),
							(short) m_JdbcUtil.getShortColumnValue(rs, 10),
					...
					jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
							"generateForeignKeys():Exception in execute(): " + e + "\n");
			...
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generateForeignKeys(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }
    ...
    public int generateStatistics(long pMemTree,int iType, long pList, Object pSearchObj)
    {
		...
		try {
			...
			try {
				...
				jdam.trace(m_tmHandle, ip.UL_TM_ERRORS,
						"Error while closing resultset in generateStatistics(): " + e + "\n");
			}
		}
        return IP_SUCCESS;
    }
    ...
    private int MapJdbcTypeToXoType(int iJdbcType)
    {
        ...
        switch(iJdbcType)
        {
			...
			case Types.BOOLEAN:      iXoType = XO_TYPE_BIT; break;
